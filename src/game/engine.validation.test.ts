...
describe('9.3 release gate: canonical rule validation',()=>{
 ...
 it('finds Ace of Spades (Hukam) holder as the starting player when state is already dealt',()=>{const ps=[p('1',[c('2','clubs')]),p('2',[c('A','spades')]),p('3',[c('K','hearts')])];expect(findStartingPlayer(ps)).toBe('2');});
 it('requires A-spades as the opening card',()=>{const ps=[p('1',[c('A','spades'),c('K','hearts')]),p('2',[c('3','clubs')]),p('3',[c('4','clubs')])];const state={...createGame(ps,createDeck()),players:ps,currentPlayerId:'1'};expect(()=>playCard(state,'1','K-hearts')).toThrow('opening card must be A♠');const next=playCard(state,'1','A-spades');expect(next.trick[0].card.id).toBe('A-spades');});
 ...
 it('keeps state immutable and rejects stale or foreign plays',()=>{const ps=[p('1',[c('A','spades')]),p('2',[c('3','clubs')]),p('3',[c('4','clubs')])];const s={...createGame(ps,createDeck()),players:ps,currentPlayerId:'1'};const before=s.players[0].hand.map(x=>x.id);const next=playCard(s,'1','A-spades');expect(s.players[0].hand.map(x=>x.id)).toEqual(before);expect(next.currentPlayerId).toBe('2');expect(()=>playCard(next,'1','A-spades')).toThrow();expect(()=>playCard(next,'2','A-spades')).toThrow();});
});
