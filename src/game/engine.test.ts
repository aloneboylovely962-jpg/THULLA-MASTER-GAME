...
describe('Thulla Master game rules',()=>{
 ...
 it('starts with the player holding 2 of clubs',()=>{const ps=[player('p1',[card('A','hearts')]),player('p2',[card('2','clubs')]),player('p3',[card('K','spades')])];expect(findStartingPlayer(ps)).toBe('p2');});
 ...
 it('rejects playing out of turn and invalid card ownership',()=>{const s=createGame([player('p1'),player('p2'),player('p3')],createDeck());expect(()=>playCard(s,'not-a-player','2-clubs')).toThrow();const current=s.currentPlayerId;expect(()=>playCard(s,current,'not-a-card')).toThrow();});
 it('removes a legal played card and advances turn',()=>{const ps=[player('p1',[card('2','clubs')]),player('p2',[card('3','clubs')]),player('p3',[card('4','clubs')])];const s={...createGame(ps,createDeck()),currentPlayerId:'p1',players:ps};const next=playCard(s,'p1','2-clubs');expect(next.players.find(p=>p.id==='p1')?.hand).toHaveLength(0);expect(next.currentPlayerId).toBe('p2');});
});
